...
class HardwareCustomizer:
    ...
    def hardware_customization(self, hardware_report, macos_version):
        ...
        self.utils.head(_("Hardware Customization"))
        ...
        for device_type, devices in self.hardware_report.items():
            ...
            if device_type == "BIOS":
                ...
                if devices.get("Firmware Type") != "UEFI":
                    print("\n" + _("*** BIOS Firmware Type is not UEFI"))
                    print("")
                    print(_("Do you want to build the EFI for UEFI?"))
                    print(_("If yes, please make sure to update your BIOS and enable UEFI Boot Mode in your BIOS settings."))
                    print(_("You can still proceed with Legacy if you prefer."))
                    print("")

                    while True:
                        answer = self.utils.request_input(_("Build EFI for UEFI? (Yes/no): ")).strip().lower()
                        if answer == "yes":
        ...
        if self.selected_devices:
            self.utils.head(_("Device Selection Summary"))
            print("")
            print(_("Selected devices:"))
            print("")
            print(_("Type          Device                                     Device ID"))
            print("------------------------------------------------------------------")
            ...
            print("")
            print(_("All other devices of the same type have been disabled."))
            print("")
    ...
    def _handle_device_selection(self, device_type):
        ...
        if len(devices) > 1:       
            ...
            if device_type == "WiFi" or device_type == "Bluetooth":
                print(_("macOS works best with only one {} device enabled.").format(device_type))
                ...
                if _apu_index or _navi_22_indices:
                    print(_("Multiple active GPUs can cause kext conflicts in macOS."))
    ...
    def _select_device(self, device_type, devices, device_groups=None):
        print("")
        if device_groups:
            print(_("Please select a {} combination configuration:").format(device_type))
        else:
            print(_("Please select which {} device you want to use:").format(device_type))
        print("")
        ...
        if device_groups:
            ...
            for idx, (group_devices, _, group_compatibility) in enumerate(valid_combinations, start=1):
                ...
                if group_compatibility:
                    print(_("   Compatibility: {}").format(self.compatibility_checker.show_macos_compatibility(group_compatibility)))
                if len(group_devices) == 1:
                    ...
                    if device_props.get("OCLP Compatibility"):
                        ...
                        if self.utils.parse_darwin_version(oclp_compatibility[0]) > self.utils.parse_darwin_version(group_compatibility[0]):
                            print(_("   OCLP Compatibility: {}").format(self.compatibility_checker.show_macos_compatibility((oclp_compatibility[0], os_data.get_lowest_darwin_version()))))
                print("")
            
            while True:
                choice = self.utils.request_input(_("Select a {} combination (1-{}): ").format(device_type, len(valid_combinations)))
            ...
            for index, device_name in enumerate(devices, start=1):
                ...
                print(_("   Device ID: {}").format(device_props.get("Device ID", "Unknown")))
                print(_("   Compatibility: {}").format(self.compatibility_checker.show_macos_compatibility(compatibility)))
            ...
            while True:
                choice = self.utils.request_input(_("Select a {} device (1-{}): ").format(device_type, len(devices)))
